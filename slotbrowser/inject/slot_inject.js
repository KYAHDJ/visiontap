...
(function () {
  ...
  function onlyHudChanges(records) {
    return records.length > 0 && records.every(record => {
      const el = record.target.nodeType === 1 ? record.target : record.target.parentElement;
      return el && el.closest && el.closest('#visiontap-hud');
    });
  }
  function scheduleAdScan(records) {
    if (onlyHudChanges(records) || adScanTimer !== null) return;
    // Coalesce page mutation bursts instead of rescanning on every DOM update.
  ...
  try {
    _adObserver = new MutationObserver(scheduleAdScan);
    if (document.body) {
  ...
  vt.fill = async (request) => {
    const options = request && typeof request === "object" ? request : {};
    const answer = typeof request === "object" && request ? request.answer : request;
    if (!answer || answer === "undefined" || answer === "null" || answer === "0" || answer === "NaN") {
      return { status: "blocked" };
    }
    return await pasteAndSubmit(String(answer).trim(), options);
  };
  ...
  function centerTaskArea() {
    ...
    if (target) {
      ...
      if (rect.height > 0 && Math.abs(middle - window.innerHeight / 2) > 24) {
        target.scrollIntoView({ behavior: 'auto', block: 'center' });
      }
    }
  }
  let centerTimer = null;
  function scheduleCenter(records) {
    if (onlyHudChanges(records) || centerTimer !== null) return;
    centerTimer = setTimeout(() => { centerTimer = null; centerTaskArea(); }, 200);
  }
